...
const usbdevsPath = "C:/Users/jakem/Downloads/usbdevs.txt";

const driverPath = "C:/Users/jakem/Downloads/if_urtwn.c";
const driverName = "urtwn";
const driverDevType = "network/wireless";
const driverBus = "USB";
const driverMatch = "URTWN_DEV_8188F";
...
function extractDeviceFields(text: string): DeviceRecord[] {
  return text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => new RegExp(`^\\s*${driverMatch}\\(`).test(line))
    .map((line) => {
      const match = line.match(new RegExp(`^\\s*${driverMatch}\\((\\S+),\\s+(\\S+)\\),\\s*$`));
      if (!match) {
        return null;
      }
...
async function findUsbdev(vendor_dev: string, device_dev: string, usbdevsText: string, driverId: bigint): Promise<void> {
  const regex = new RegExp(`^product\\s+${vendor_dev}\\s+${device_dev}\\s+(\\S+)\\s+(\\S+)`, 'm');
